...
def article_to_ids(article_words, vocab):
    # 给每一个传过来的句子进行word2id的表示，常规的表示可能出现oov的情况，这里使用了动态的维护新出现词的oovs词典。
    # 编码后的句子id中不存在unk_id的编号
    ids = []
    oovs = []
...
def example_generator(params, vocab, max_enc_len, max_dec_len, mode, batch_size):
    if mode != "test":
        dataset_1 = tf.data.TextLineDataset(params[f"{mode}_seg_x_dir"])  # train_X_seg_data
        dataset_2 = tf.data.TextLineDataset(params[f"{mode}_seg_y_dir"])  # train_Y_seg_data
        train_dataset = tf.data.Dataset.zip((dataset_1, dataset_2)).take(100000)  # 从该数据集中创建一个最多包含100000个元素的数据
        ...
        for raw_record in train_dataset:
            ...
            start_decoding = vocab.word_to_id(Vocab.START_DECODING)  # 获取开始与结束的字典中的id
            stop_decoding = vocab.word_to_id(Vocab.STOP_DECODING)

            article_words = article.split()[:max_enc_len]  # 截取最大长度
            ...
            if params['pointer_gen']:
                abs_ids_extend_vocab = abstract_to_ids(abstract_words, vocab, article_oovs)  # abstract_to_ids函数根据编码端动态创建的article_oovs对摘要中oov的词进行word2id
...
def get_steps_per_epoch(params):
    ...
    if params['decode_mode'] == 'beam':
        return num_examples
    steps_per_epoch = math.ceil(num_examples // params['batch_size'])  # ceil返回大于该数字的最小整数
