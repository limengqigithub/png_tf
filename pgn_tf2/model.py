...
class PGN(tf.keras.Model):
    ...
    def call(self, dec_input, dec_hidden, enc_output,
        ...
        final_dists = _calc_final_dist(enc_extended_input,
                                       ...
                                       attentions,
                                       p_gens,
                                       batch_oov_len,  # batch_oov_len 文章中对应的oov单词数量
                                       self.vocab_size,
                                       self.batch_size)
        attentions = tf.stack(attentions, axis=1)  # attentions shape=(16, 40, 200) is means b, dec_seq_len, en_seq_len
...
def _calc_final_dist(_enc_batch_extend_vocab, vocab_dists, attn_dists, p_gens, batch_oov_len, vocab_size, batch_size):
    ...
    attn_len = tf.shape(_enc_batch_extend_vocab)[1]  # number of states we attend over  == 200
    batch_nums = tf.tile(batch_nums, [1, attn_len])  # shape (batch_size, attn_len)  第一个样本全是0，最后一个样本全是batch_size-1
